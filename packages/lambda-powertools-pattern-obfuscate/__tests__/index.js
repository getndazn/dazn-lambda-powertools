...
const MockSampleLogging = jest.mock(
  "@buyerassist/dazn-lambda-powertools-middleware-sample-logging",
  () => {
...
const MockCaptureCorrelationIds = jest.mock(
  "@buyerassist/dazn-lambda-powertools-middleware-correlation-ids",
  () => {
...
describe("obfuscate pattern", () => {
  describe("when there are no overrides", () => {
    it("should set sample debug log rate to 0.01", async () => {
      ...
      await handler({}, {});
      expect(CaptureCorrelationIds).toHaveBeenCalledWith({
        sampleDebugLogRate: 0.01,
      });
      expect(SampleLogging).toHaveBeenCalledWith({
        sampleRate: 0.01,
        obfuscationFilters: expect.anything(),
      });
    });
  });
  ...
  describe("when there are predefined DATADOG_TAGS", () => {
    ...
    const expectedTags =
      "awsRegion:__region__,functionName:,functionVersion:,environment:test,custom:tag,key:value";
