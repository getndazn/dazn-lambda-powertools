...
const {
  ...
  FILTERING_MODE: obfuscaterFilteringMode,
} = require("@buyerassist/dazn-lambda-powertools-middleware-obfuscater");
...
const errorObfuscater = (obfuscationFilters, filteringMode) =>
  obfuscaterMiddleware({
    obfuscationFilters,
...
const genericObfuscater = (obfuscationFilters, filteringMode, filterOnAfter) =>
  obfuscaterMiddleware({
    obfuscationFilters,
    filteringMode: toObfuscaterFilteringMode(filteringMode),
    filterOnBefore: false,
...
const obfuscaterPattern = (
  obfuscationFilters,
  f,
  filterOnAfter = false,
  filteringMode = FILTERING_MODE.BLACKLIST
) => {
  return (
    middy(f)
      .use(
