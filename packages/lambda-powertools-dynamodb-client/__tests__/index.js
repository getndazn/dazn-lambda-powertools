...
describe("DynamoDB client", () => {
    ...
    describe("when there are no correlation IDs", () => {
      it("sends empty __context__", async () => {
        ...
        const expectations = {
          ...
          names: {
            "#count": "count",
            "#LambdaPowertoolsContext": "__context__",
          },
  ...
  describe(".updateWithCorrelationIds", () => {
    it("forwards given correlationIds in __context__ field", async () => {
      ...
      const expectations = {
        ...
        values: {
          ":count": 1,
          ":LambdaPowertoolsContext": correlationIds.get(),
        },
      };
  ...
  describe(".transactWriteWithCorrelationIds", () => {
    it("forwards given correlationIds in __context__ field", async () => {
      ...
      await verifyTransactWriteWithCorrelationIds(
        correlationIds,
        input,
        expectations
      );
...
async function verifyPut(correlationIds) {
  ...
  const expectedItem = Object.assign({}, item, { __context__: correlationIds });
...
async function verifyUpdateWithCorrelationIds(
  correlationIds,
  input,
  expectations
) {
...
async function verifyTransactWriteWithCorrelationIds(
  correlationIds,
  input,
  expectations
) {
  ...
  await DynamoDB.transactWriteWithCorrelationIds(
    correlationIds,
    params
  ).promise();
