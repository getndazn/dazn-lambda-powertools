...
const verifyPutRecordBatchContext = async (f) => {
  ...
  const records = eventTypes.map((eventType) => {
...
const verifyPutRecordBatchWithCorrelationIdsContext = async (
  correlationIds,
  f
) => {
  const eventTypes = ["wrote_test", "ran_test", "pass_test"];
  ...
  await Firehose.putRecordBatchWithCorrelationIds(
    correlationIds,
    params
  ).promise();
...
describe("Firehose client", () => {
  ...
  describe(".putRecordBatchWithCorrelationIds", () => {
    it("forwards given correlationIds in __context__ field", async () => {
      ...
      await verifyPutRecordBatchWithCorrelationIdsContext(
        correlationIds,
        (x) => {
          expect(x).toEqual({
            "x-correlation-id": "child-id",
